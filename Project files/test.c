...
/**************************************************************************
 ...
 Test creates files more than maximum number of files.
 Test tries to open files
	1.existent file
	2.Non-existent file
	3.Already opened file.
 Test tries to close file
	1.existent open file
	2.non open file
	3.Non-existent file.
...
void test2i(void) {
	...
	char *filedata;
	int i=0;
	printf("This is Release %s:  Test 2i\n", CURRENT_REL);
	CREATE_FILE("Sample.txt", &Z502_REG9);
		SuccessExpected(Z502_REG9, "CREATE_FILE");
	
	Z502_REG9 = ERR_SUCCESS;
	Z502_REG3 = -1;
	while (Z502_REG9 == ERR_SUCCESS) {
		Z502_REG3++; /* Generate next unique program name*/
		sprintf(process_name, "Test2i_%ld", Z502_REG3);
		printf("Creating file \"%s\"\n", process_name);
		CREATE_FILE(process_name, &Z502_REG9);
	}
	ErrorExpected(Z502_REG9, "CREATE_FILE");
	printf("%ld files were created in all.\n\n", Z502_REG3);

	//open existent file
	OPEN_FILE("Sample.txt", &Z502_REG9);
		SuccessExpected(Z502_REG9, "OPEN_FILE");
	//Error should have occured because no such file exists12/11/2013
	OPEN_FILE("a.txt", &Z502_REG9);
		ErrorExpected(Z502_REG9, "OPEN_FILE");
	//error should have occured because file is already opened.
	OPEN_FILE("Sample.txt", &Z502_REG9);
		ErrorExpected(Z502_REG9, "OPEN_FILE");

	//close open file
	CLOSE_FILE("Sample.txt", &Z502_REG9);
	SuccessExpected(Z502_REG9, "CLOSE_FILE");
	//error should have occured because file is already closed.
	CLOSE_FILE("Sample.txt", &Z502_REG9);
	ErrorExpected(Z502_REG9, "CLOSE_FILE");
	//error should have occured because file does not exists
	CLOSE_FILE("a.txt", &Z502_REG9);
	ErrorExpected(Z502_REG9, "CLOSE_FILE");
